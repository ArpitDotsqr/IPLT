...
import {
  getFieldExecutiveTripDetail,
  getManagerUserDataById,
} from "@/redux/actions/manageuser/manageruserlist";
...
const ManagerUserDetailPage = () => {
  ...
  const managerUserDataById = useSelector(
    (state) => state.managerUserSlice?.particularmanagerUserData
  );
  const LoaderManageUser = useSelector(
    (state) => state.managerUserSlice.isLoading
  );

  const allTripExeGraphCount = useSelector(
    (state) => state.managerUserSlice.allTripExecutiveGraphCount
  );
  const completedTripExeCount = useSelector(
    (state) => state.managerUserSlice.completedExecutiveGraphCount
  );
  const ongoingTripExeCount = useSelector(
    (state) => state.managerUserSlice.ongoingExecutiveGraphCount
  );

  const manageUserCountData = useSelector(
    (state) => state.tripSlice?.tripCount
  );
  ...
  const allTripExeData = FuncData(
    "",
    [ongoingTripExeCount, completedTripExeCount],
    ["#165DFF", " #50CD89"]
  );
  const onGoingTripsExeData = FuncData(
    "",
    [ongoingTripExeCount, completedTripExeCount],
    ["#165DFF", "#F2F2F2"]
  );
  const completedTripsExeData = FuncData(
    "",
    [ongoingTripExeCount, completedTripExeCount],
    ["#F2F2F2", "#50CD89"]
  );
    ...
    <>
      ...
      <div className="manageUser_Section ManageUserDetailsSection">
        <div class="back_btn_all">
          <p onClick={() => router.push("/admin/manageusers")}>
        ...
        <div className="manger_edit_details_main manger_user_main">
          <div className="edit_manger_active d-flex justify-content-end ">
            ...
            {managerUserDataById.Status &&
            managerUserDataById.Status.key === "pending" ? (
              <div className="user_data_left d-flex align-items-center">
                <span className="filed_admin_text">
                  {managerUserDataById.Status.key}
                </span>
              </div>
            ) : (
              <>
                <Form>
                  <Form.Check
                    ...
                    onClick={() => {
                      if (isUpdated[managerUserDataById?.id]) {
                        ...
                        if (managerUserDataById?.Status?.key === "active") {
                          ...
                          managerUserDataById?.Status?.key === "inactive"
                        ) {
                          handleActive(managerUserDataById?.id, "active");
                        }
                      }
                    ...
                    checked={
                      isUpdated[managerUserDataById?.id]
                        ? isUpdated[managerUserDataById?.id] === "active"
                          ? true
                          : false
                        : managerUserDataById?.Status?.key === "active"
                    }
                  />
                </Form>
            ...
            <Button
              ...
              onClick={() =>
                router.push(
                  `/admin/manageusers/edit/${managerUserDataById?.id}`
                )
              }>
              Edit Details
            </Button>
          ...
          <div className="manager_edit_detail_inner">
            ...
            <div className="ManageUserTabal">
              ...
              <div className="ManageUserTabalRow">
                ...
                <p>
                  {managerUserDataById?.UserDetail?.alternateMobile
                    ? managerUserDataById?.UserDetail?.alternateMobile
                    : "-"}
                </p>
              </div>
              ...
              <div className="ManageUserTabalRow">
                ...
                <p>
                  {managerUserDataById?.UserDetail?.aadhaar
                    ? managerUserDataById.UserDetail.aadhaar.replace(
                        /(\d{4})(\d{4})(\d{4})/,
                        "$1 $2 $3"
                      )
                    : "-"}
          ...
          <div class="manager_edit_detail_inner">
            ...
            <div className="ManageUserTabal">
              <div className="ManageUserTabalRow">
                ...
                <p
                  className="spanDecoration"
                  style={{ cursor: "pointer" }}
                  onClick={() =>
                    handleView(managerUserDataById?.UserDocuments?.aadhaar)
                  }>
                  {managerUserDataById?.UserDocuments?.aadhaar
                    ? managerUserDataById?.UserDocuments?.aadhaar
                    : "-"}
                </p>
              </div>
              <div className="ManageUserTabalRow">
                ...
                <p
                  ...
                  onClick={() =>
                    handleView(
                      managerUserDataById?.UserDocuments?.panCardFrontImage
                    )
                  }>
                  {managerUserDataById?.UserDocuments?.panCardFrontImage
                    ? managerUserDataById?.UserDocuments?.panCardFrontImage
              ...
              <div className="ManageUserTabalRow">
                ...
                <p
                  ...
                  onClick={() =>
                    handleView(
                      managerUserDataById?.UserDocuments?.cancelChequeImage
                    )
                  }>
                  {managerUserDataById?.UserDocuments?.cancelChequeImage
                    ? managerUserDataById?.UserDocuments?.cancelChequeImage
          ...
          <div class="manager_edit_detail_inner">
            ...
            <div className="ManageUserTabal">
              {managerUserDataById.ProjectStaffData?.length > 0
                ? managerUserDataById.ProjectStaffData.reduce(
                    (uniqueProjects, item) => {
                      if (
                        !uniqueProjects.find(
                          (project) =>
                            project.ProjectData.id === item.ProjectData.id
                        )
                      ) {
                ...
                <div className="manger_edit_image_bar_main row justify-content-around">
                  <div className="manger_edit_image_bar_inner col-sm-2">
                    ...
                    <div className="m-4">
                      <h5>All Trips</h5>
                      <h2>
                        {allTripExeGraphCount < 10
                          ? `0${allTripExeGraphCount}`
                          : `${allTripExeGraphCount}`}
                      </h2>
                    </div>
                  </div>
                  ...
                  <div className="manger_edit_image_bar_inner col-sm-2">
                    ...
                    <div className="m-4">
                      ...
                      <h2>
                        {ongoingTripExeCount < 10
                          ? `0${ongoingTripExeCount}`
                          : `${ongoingTripExeCount}`}
                      </h2>
                    </div>
                  </div>
                  ...
                  <div className="manger_edit_image_bar_inner col-sm-2">
                    ...
                    <div className="m-4">
                      ...
                      <h2>
                        {completedTripExeCount < 10
                          ? `0${completedTripExeCount}`
                          : `${completedTripExeCount}`}
                      </h2>
                    </div>
                  </div>
                </div>
